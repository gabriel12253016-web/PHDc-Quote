...
with header_col1:
    if os.path.exists("logo.svg"):
        with open("logo.svg", "r", encoding="utf-8") as f:
            st.markdown(f'<div style="width:70px">{f.read()}</div>', unsafe_allow_html=True)
...
with st.sidebar:
    st.subheader("Population Health Data Center NCKU")
    st.markdown("""
    📞 +886-6-2353535 ext.6820
...
with col_left:
    ...
    # 設計備註
    if "高階因果推論與複雜模型" in design_sel:
        st.markdown('<div class="caption-text">內容包含：Self-controlled (SCCS, CCO)、TND (陰性對照)、ITS、TTE (Sequential/Clon等)、工具變數 (IV)、RDD、Trend in trend等...</div>', unsafe_allow_html=True)
        st.markdown('<div class="caption-text">⚠️ 提醒：因選擇與實際最終使用可能有落差，最後計算多出價差將於第三期支付。</div>', unsafe_allow_html=True)
    elif design_sel:
        st.markdown('<div class="caption-text">內容包含：基礎統計描述、單變項分析、多因素迴歸、傾向分數配對、存活分析、共變項調整等...</div>', unsafe_allow_html=True)
    ...
    if work_choice != "僅諮詢 (不含資料庫串聯)":
        ...
        if use_nhird:
            st.markdown(f'<div class="caption-text">包含項目：{", ".join(st.session_state.db_nhird_df["名稱"].tolist()[:5])}...</div>', unsafe_allow_html=True)
            # 計算費用 (維護費 + 20% 購買費)
        ...
        if use_ehr:
            st.markdown(f'<div class="caption-text">包含項目：{", ".join(st.session_state.db_ehr_df["名稱"].tolist())}</div>', unsafe_allow_html=True)
...
with col_right:
    ...
    st.markdown("---")
    st.write("**售後支援額度 (Scope Creep)**")
    st.write(f"- 模型微調/次分組分析：{n_tune} 次")
    st.write(f"- 研究假說變更/重分析：{n_reanalysis} 次")
    st.write(f"- 文稿大修 (需購撰寫服務)：{n_revise} 次")
...
if submit_btn:
    if all([u_name, u_org, u_phone, u_email]):
        ...
        save_details = f"掛名：{auth_summary} | 調校：{n_tune}/{n_reanalysis}/{n_revise} | 提醒：{design_msg if design_msg else '無'}"
        conn = sqlite3.connect('phdc_orders.db')
