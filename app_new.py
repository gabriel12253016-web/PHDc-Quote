...
st.markdown("""
    ...
    [data-testid="column"]:nth-child(2) {
        position: sticky;
        top: 2rem;
        height: calc(100vh - 4rem); /* 限制高度在視窗內 */
        overflow-y: auto; /* 如果內容太長，右側內部分別捲動 */
        z-index: 999;
    }

    /* 2. 確保父層容器不會限制 sticky 效果 */
...
if is_admin:
    ...
    with t1:
        ...
        st.write("1. **基礎成本** = 固定維持費 + Σ(所選資料庫維護費 + 購買費 × 0.2)")
...
with col_right:
    ...
    # --- 動態公式呈現 (新增處) ---
    # 這裡會根據目前的變數自動帶入數字
    formula_display = f"({base_cost:,.0f} + {labor_total * sum_k:,.0f}) × {f_total_adj:.2f}"
    st.info(f"💡 總額 = {formula_display} = {total_cost:,}")
    # --------------------------
